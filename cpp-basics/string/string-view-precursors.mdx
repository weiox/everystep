...
<Mermaid chart={`
graph LR
    subgraph "一个完整的 std::string 📚 (拥有者)"
        A["<b>s (对象)</b><br/>在栈上<br/>{指针, size, capacity}"] --> B["<b>['H','e','l','l','o',' ','W','o','r','l','d']</b><br/>在堆上"];
    end

    subgraph "一个“切片”对象 🔖 (观察者)"
        C["<b>sv (对象)</b><br/>在栈上<br/>{ptr: B+6, len: 5}"] -.-> D["'W','o','r','l','d'"];
    end
